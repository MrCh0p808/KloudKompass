...
def parse_tags(tag_args) -> dict:
    """
    Parse CLI tag arguments into a dict.

    Accepts a list of 'key=value' strings. If no '='  is present, the
    value is set to empty string (tag-key-only filter).

    Args:
        tag_args: List of 'key=value' strings, or None.

    Returns:
        Dict of tag key → value mappings.
    """
    if not tag_args:
        return {}
